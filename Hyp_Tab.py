...
for i, e in enumerate(cat_names):
    p_tot[i] = np.sum(
        pacific.Count[cats[e][0] <= pacific.Elevation][pacific.Elevation <= cats[e][1]]
    )
    a_tot[i] = np.sum(
        atlantic.Count[cats[e][0] <= atlantic.Elevation][
            atlantic.Elevation <= cats[e][1]
        ]
    )
    i_tot[i] = np.sum(
        indian.Count[cats[e][0] <= indian.Elevation][indian.Elevation <= cats[e][1]]
    )
    s_tot[i] = np.sum(
        southern.Count[cats[e][0] <= southern.Elevation][
            southern.Elevation <= cats[e][1]
        ]
    )
tab_df = pd.DataFrame(
    {"Pacific": p_tot, "Atlantic": a_tot, "Indian": i_tot, "Southern": s_tot},
    index=cat_names,
)
print(tab_df)
